...
def audio_modal():
    ...
    modal_out = Dense(9, activation="sigmoid")(modal_2_Dense_1)
    
    model = Model(inputs=[modal_2_x_in, modal_2_a_in], outputs=modal_out)
    return model
...
for trial in tqdm(range(n_trial)):
    ...
    def train():
        with tf.GradientTape() as tape:
            predictions = model([audio_graph.x, audio_graph.a], training=True)
            predictions_tr = tf.math.multiply(predictions, tf.cast(mask_tr, tf.float32))
            y_tr = tf.math.multiply(audio_graph.y, mask_tr)
    ...
    def evaluate(): 
        ...
        for mask in [mask_tr, mask_va, mask_te]:
            ...
            mae = tf.reduce_mean(mean_absolute_error(audio_graph.y[mask], predictions[mask]))
            mae_lst.append(mae)
