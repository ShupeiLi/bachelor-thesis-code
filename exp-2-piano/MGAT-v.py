...
def video_modal():
    ...
    modal_out = Dense(9, activation="sigmoid")(modal_1_Dense_1)
    
    model = Model(inputs=[modal_1_x_in, modal_1_a_in], outputs=modal_out)
    return model
...
for trial in tqdm(range(n_trial)):
    ...
    def train():
        with tf.GradientTape() as tape:
            predictions = model([video_graph.x, video_graph.a], training=True)
    ...
    @tf.function
    def evaluate(): 
        predictions = model([video_graph.x, video_graph.a], training=False)
        losses = []
        zero_one_lst = []
        mae_lst = []
