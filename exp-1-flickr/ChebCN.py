...
# Parameters
channels_1 = 512 # Number of channels in each head of the first GAT layer
n_epoch = 1000  # Number of training epochs
n_patience = 10  # Patience for early stopping
l2_reg = 2.5e-4  # L2 regularization rate
order = 2 # K Chebyshev polynomials
seeds = 42
n_trial = 20
dropout = 0.01
...
def two_modals():
    ...
    modal_1_gcn_1 = ChebConv(channels_1,
                            K=order,
                            activation="sigmoid",
                            kernel_regularizer=l2(l2_reg),
                            bias_regularizer=l2(l2_reg)
                            )([modal_1_x_in, modal_1_a_in])
    modal_1_gcn_1 = Dropout(dropout)(modal_1_gcn_1)
    modal_1_Dense_1 = Dense(32, activation="ReLU")(modal_1_gcn_1)
    
    # Structure branch
    modal_2_x_in = Input(shape=(str_data.n_node_features,))
    modal_2_a_in = Input(shape=(str_data.n_nodes,))
    
    modal_2_gcn_1 = ChebConv(channels_1,
                            K=order,
                            activation="sigmoid",
                            kernel_regularizer=l2(l2_reg),
                            bias_regularizer=l2(l2_reg)
                            )([modal_2_x_in, modal_2_a_in])
    modal_2_gcn_1 = Dropout(dropout)(modal_2_gcn_1)
    modal_2_Dense_1 = Dense(32, activation="ReLU")(modal_2_gcn_1)
