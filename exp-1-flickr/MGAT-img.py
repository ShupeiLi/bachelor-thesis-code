...
def img_unimodal():
    ...
    modal_1_out = Dense(1)(modal_1_Dense_1)
    
    model = Model(inputs=[modal_1_x_in, modal_1_a_in], outputs=modal_1_out)
    return model
...
for trial in tqdm(range(n_trial)):
    ...
    def train():
        with tf.GradientTape() as tape:
            predictions = model([img_graph.x, img_graph.a], training=True)
    ...
    @tf.function
    def evaluate():
        predictions = model([img_graph.x, img_graph.a], training=False)
        losses = []
        mse_lst = []
        mae_lst = []
